...
こういった「方程式が定める図形」すなわち\textbf{代数多様体}(\textit{algebraic variety})を、「方程式の形だけから調べよう」というのが代数幾何学です。

このような問題には、まず素朴に次のような問題を考えることができます。
\begin{problem}
  $x,y$を実数とする。このとき、
  \[
    (x^2+y^2)^2-(x^2-y^2)=0
...
\end{problem}
これは\textbf{レムニスケート}と呼ばれる、二つの穴がある図形になっています。
...
\begin{itemize}
  \item ある体$K/k$を固定したとき、$K$を関数体にもつ代数曲面$K(X)$の集合は、双有理射について半順序集合をなす。
  \item $K(\mathbb{P}^2)$や、代数曲線$C$に対して$K(\mathbb{P}^1\times C)$という形の代数曲面には、2つ以上の極小元が存在する。
...
\[
  C_k:x^k+y^k=1
\]
という方程式を満たす非自明な点、つまり$xy=0$を満たす点以外は存在しないだろう、と問い直すことができます。
この問題を少し変えて、$\mathbb{F}_q$上で考えたFermat曲線を$C_k(\mathbb{F}_q)$と書くとき、そこに乗っている点の個数$N_1=\#(C_k(\mathbb{F}_q))$を考えることには単純に興味がわきます。
このように代数多様体上の$\mathbb{F}_q$有理点の個数というのは、Fermat予想を背景にして数学者の興味を引きつつけていました。
...
$X=X(\mathbb{F}_q)$でもこのアナロジーを考えることができ、$X$の各\textbf{閉点}$x$\footnote{
  現代の代数幾何学では、代数多様体は可換環から作られるスキームという空間の貼り合わせで定義され、閉点はその極大イデアルに対応します。
...
最後の和
\[
  \sum_{x\in|X|,\deg(x)\mid n}\deg(x)
\]
を考えましょう。
実はこれこそ$N_n=\#(X(\mathbb{F}_{q^n}))$に他なりません。

これを理解するために、$x\in|X|$をひとつ取り出して考えてみます。
先ほども述べた通り、閉点$x$というのは$X$から定義される最小多項式のようなものです(実際には極大イデアル)。
また、$d:=\deg(x)=[k(x):\mathbb{F}_q]$とおけば、$x$は$X(\mathbb{F}_{q^d})$の元とみなせます。
ここで体論の一般論により、
\begin{itemize}
  \item $d\nmid n$ならば$k(x)\cong\mathbb{F}_{q^d}$は$\mathbb{F}_{q^n}$に埋め込めないです。\\
  これは、この$x$に対応する$\mathbb{F}_{q^n}$の点が存在しないことを意味します。
  \item $d\mid n$ならば$\mathbb{F}_{q^d}$は$\mathbb{F}_{q^n}$に埋め込めます。\\
  その相異なる埋め込み方は、ちょうど$d=[k(x):\mathbb{F}_q]$個存在します。
  これらの埋め込みに対応する$\mathbb{F}_{q^n}$の有理点が存在します。
\end{itemize}
従って
\[
...
どういうことかというと、まず$H^r({\rm Frob}_q^n)$を$({\rm Frob}_q^n)^*:H^r(X)\to H^r(X)$のある表現行列だとすると、
\[
  N_n=\sum_{r}(-1)^r{\rm Tr}(H^r({\rm Frob}_q^n))
\]
になるということです。
これを用いて合同ゼータ関数を計算してみましょう。
\begin{align*}
  \log(Z(X,t))&=\sum_{n=1}^\infty\frac{N_nt^n}{n}\\
  &=\sum_{n=1}^\infty\sum_{r}(-1)^r\frac{{\rm Tr}(H^r({\rm Frob}_q^n))t^n}{n}\\
  &=\sum_r(-1)^r\sum_{n=1}^\infty\frac{{\rm Tr}(H^r({\rm Frob}_q^n))t^n}{n}\\
\end{align*}
さて
\[
  \sum_{n=1}^\infty\frac{{\rm Tr}(H^r({\rm Frob}_q^n))t^n}{n}
\]
の部分ですが、もっと綺麗に整理できます。
まず行列とみた線形写像$H^r({\rm Frob}_q^n)$のトレースというのは、その固有値の和と一致するのでした。
これらを$\lambda_1,\dots,\lambda_m$とすると、$H^r({\rm Frob}_q^n)$は$H^r({\rm Frob}_q)$を$n$回合成したものですから、その固有値は$\lambda_1^n,\dots,\lambda_m^n$となります。
従って
\[
  {\rm Tr}(H^r({\rm Frob}_q^n))=\lambda_1^n+\cdots+\lambda_m^n
\]
となります。
ゆえに
\begin{align*}
...
\end{align*}
そして$1-H^r({\rm Frob}_q^n)t$という行列を対角化して行列式をとったと考えれば、
\[
  \sum_{n=1}^\infty\frac{{\rm Tr}(H^r({\rm Frob}_q^n))t^n}{n}=-\log\left(\det(1-H^r({\rm Frob}_q^n)t)\right)
\]
と書けます。
ここで、$t$の多項式$\det(1-H^r({\rm Frob}_q^n)t)$を
\[
  P_r(t):=\det(1-H^r({\rm Frob}_q^n)t)
\]
とおくと、結局
\begin{align*}
  \log(Z(X,t))&=\sum_{n=1}^\infty\frac{N_nt^n}{n}\\
...
そのためにGrothendieckはスキームというものを導入して、代数幾何学を行う「場」としてスキームを定義しました。
...
\begin{theorem}
  ...
  \]
  を対応させる対応は、「$k$上のアフィン多様体の圏」から「有限生成$k$-整域」への反変な圏同型を誘導する。
\end{theorem}
このことから、$X$の研究をするということと、その座標環$A(X)$を研究することは同じことだとわかります。
この事実から、スキームをどう定義するべきかという問いに自然な答えが得られます。
TODO
