\chapter{可換環論 入門}\label{chapter-commutative_rings_introduction}

Hartshorneに出てくる環論の中でも、僕の独断と偏見で基礎的と思われるものを頑張って解説する章です。
...
などとぬかしおるので、環の定義から始めて「\textbf{Noether環の任意のイデアルは有限生成}」まで証明しないと、2ページ目で詰むわけです。
ですが、Noether環なんていくらでも語ることがあるので、とても一つの章に収まるものではないです。

とりあえずこの章では、環準同型定理までを丁寧にやっていきます。
この辺は\mycite{Hot21}の第一章をかなり参考にしました。
僕も代数学は最初にこの本で勉強したもので、初学者にとって非常に分かりやすいと思います。
Hartshorneに取り組む前段の可換環論といえば\mycite{At-Mc-translate}が有名ですが、この章はもっと前の段階のものです。
...
\begin{definition}[環]
  ...
  このとき、$(A,+,\cdot,0,1)$が\textbf{環}(単に$A$と表記する)であるとは、任意の$a,b,c\in A$に対して次を満たす時を言う。
  \begin{itemize}
...
積の非可換性を満たさない環を考えることがあり、そちらは\textbf{非可換環}と呼ばれます。
僕たちは基本的に\textbf{単に環といえば必ず可換環}を指すことにします。
もし非可換環を考えることになった場合(無いと思いたいが)、必ず「非可換環」と呼ぶことにします。

また、基本的に$1=0$の可能性を排除しません。
もし$1=0$ならば、次のことが成り立ちます。
\begin{theorem}
  $A$を環とする。もし$1=0$ならば、$A=\{0\}$。
\end{theorem}
\begin{proof}
  任意の$a\in A$に対して、
  \[
...
\begin{example}
  整数全体$\mathbb{Z}$、有理数全体$\mathbb{Q}$、実数全体$\mathbb{R}$、複素数全体$\mathbb{C}$は環。
  自然数全体$\mathbb{N}$は環ではない。
\end{example}

\begin{example}
  環$A$に対して、$n$個の未知変数$x_1,\dots,x_n$の有限項からなる多項式がなす全体の集合
  \[
...
\begin{example}
  ...
  \]
  は環である。
  一般に、多項式$f(x)\in\mathbb{Q}[x]$の根$\alpha\in\mathbb{C}$を$\mathbb{Z}$に添加した$\mathbb{Z}[\alpha]$は、環である。
\end{example}
このようなものは、おそらくFermat予想へのラメ(Lam\'e)やクンマー(Kummer)の試みとして始めに考えられました。
彼らは奇素数$p$に対して、$1$の原始$p$乗根を添加した環$\mathbb{Z}[\zeta_p]$の中で
\[
...
\begin{definition}
  ...
  環$A,B$の間に同型写像が存在するとき、$A$と$B$は互いに\textbf{同型}であるという。
\end{definition}
...
\begin{definition}[イデアル]
  $A$を環とする。部分集合$\mathfrak{a}\subset A$が$A$の\textbf{イデアル}であるとは、次を満たす時を言う。
  \begin{itemize}
...
\begin{example}
  ...
  \]
  は、環$\mathbb{Z}$のイデアルである。
\end{example}

上記の例で既に用いてしまいましたが、環のいくつかの元が生成するイデアルというものを考えることができます。
\begin{theorem}
  $A$を環、$T\subset A$を$A$の部分集合とする。
  このとき、
  \[
...
\begin{definition}
  $A$を環とする。イデアル$\mathfrak{m}\subset A$が\textbf{極大イデアル}であるとは、次を満たす時を言う。
...
\begin{proof}
  ...
  ここでEuclidの互除法より$pn+xy=1$をみたす$n,y\in\mathbb{Z}$が存在し、$p,x\in\mathfrak{a}$であるから、$1=pn+xy\in\mathfrak{a}$となる。
...
\begin{theorem}
  $A$を環、$\mathfrak{a}\subset A$をイデアルとする。もし$1\in\mathfrak{a}$ならば、$\mathfrak{a}=A$
\end{theorem}
\begin{proof}
...
実はこの逆も成り立ちます。
\begin{theorem}
  $A$を環とする。このとき、$A$が体であることと、$A$のイデアルが$(0)$と$(1)$しかないことは同値である。
\end{theorem}
\begin{proof}
...
雑に言って、環の素イデアルを既約多項式のゼロ点集合とみなし、極大イデアルを1点集合とみなします。
イデアルは環があれば定義できるので、ここに一般の環を幾何学の舞台に引きずり出すことができるのです！
...
一般に、極大イデアルは素イデアルです。
\begin{theorem}
  $A$を環とする。イデアル$\mathfrak{m}\subset A$が極大イデアルならば、$\mathfrak{m}$は素イデアルである。
\end{theorem}
\begin{proof}
  背理法によって証明する。
...
\begin{example}
  $\mathbb{Z}/n\mathbb{Z}$は$n$個の元からなる環である。
  \[
...
自然な準同型$\pi:A\to A/\mathfrak{a}$の何が自然かっていうのは、まあ色々ありますが\footnote{
  ...
  環とイデアルのペアがなす圏$C$と環の圏$D$における忘却関手から始まる左随伴の列$((A,I)\mapsto A/I)\dashv (A\mapsto(A,0))\dashv (\text{忘却関手})$としてのですねｗｗｗ
...
\begin{theorem}
  $A$を環、$\mathfrak{a}$をそのイデアル、$\pi:A\to A/\mathfrak{a}$を自然な準同型とする。
...
\begin{theorem}
  $A$を環、$\mathfrak{a}$をそのイデアル、$\pi:A\to A/\mathfrak{a}$を自然な射影とする。
...
ひとつ、使い道を教えましょう。
環のイデアルが素イデアルかどうか、あるいは極大イデアルかどうかを、剰余環の性質から判定することができます。
...
他の環やイデアルにこの判定法をうまく運用するには、次の環準同型定理を知っておくとよいでしょう。
...
\end{proof}
また、環$\mathbb{R}[X]/(X^2+1)$が体$\mathbb{C}$と同型になったことで、イデアル$(X^2+1)$が実は極大イデアルであることもわかりました。
