...
可換環は一般に積の逆元を持たないとは言いましたが、決して持たないとは言っていません。
実際、体を可換環のうち、0以外の元は積の逆元を持つものと定義したわけです。
そのように、可換環の中でも積の逆元をもつものを単元といいます。
\begin{definition}
  $A$を可換環とする。$a\in A$が\textbf{単元}(\textit{unit})であるとは、ある元$b\in A$が存在して、$ab=1$を満たす時をいう。
  $A$の単元全体の集合を$A^\times$と書き、\textbf{単元群}(\textit{unit group})という。
\end{definition}
可換環$A$が体であるというのは、つまり$A^\times=A\setminus\{0\}$であるときということなんですね。
次の定理は簡単です。
\begin{theorem}
  $A$を可換環とする。このとき単元群$A^\times$は$A$の積によって可換群をなす。
\end{theorem}
\begin{proof}
  結合法則は可換環の定義から明らか。
  単位元も可換環の定義から明らかに$1\in A^\times$である。
  逆元の存在は単元群の定義から明らか。
\end{proof}
...
整域、ユークリッド環、PID、Noether環と色々な環を紹介しましたが、まだ特筆すべき種類の可換環があります。

またまたFermat予想に対するLam\'eとKummerの試みに戻ります。
彼らが直面した大きな壁は、結局のところ$\mathbb{Z}[\zeta_{23}]$における素因数分解の一意性が成り立たないことでした。
そこで可換環には、素因数分解の一意性が成り立つか否か、という分類が生まれることになります。
そのような分類を行う上で最初に問題になるのは、一般の可換環で素数に対応する「元」は何か？ということです。
...
であれば、定義としてはより広く取って、既約元を軸に素因数分解の一意性が成り立つ環を定義するのが良さそうです。
整数の素因数分解の一意性の自然な拡張としてもこちらが良さそうですね。
\begin{definition}
  整域$A$が\textbf{一意分解整域}(\textit{unique factorization domain}, 略してUFD)であるとは、任意の$a\in A\setminus\{0\}$について、$a\notin A^\times$ならば、以下を満たす時をいう。
  \begin{itemize}
    \item ある既約元$p_1,\dots,p_n$が存在して、$a=p_1\cdots p_n$を満たす
    \item 上記の分解は単元倍を除いて一意である。
    すなわち、別の既約元分解$a=q_1\cdots q_m$が存在する場合、$n=m$かつ、必要ならば番号を付け替えて$q_i=u_ip_i$ (${}^\exists u_i\in A^\times$)を満たす
  \end{itemize}
\end{definition}

先ほど既約元と素元は違うものだと言いましたが、なんとUFDでは同じものになります。
素元が既約元であることは一般の整域で成り立つので、以下を示せば十分です。
\begin{theorem}
  $A$をUFD、$p\in A$は既約元であるとする。
  このとき$p$は素元である。
\end{theorem}
\begin{proof}
  $ab\in(p)$、すなわち$ab=pk$ (${}^\exists k\in A$)とする。
  ここで、$a,b,k$を既約元に分解する。
  \begin{align*}
    a&=q_1\cdots q_m\\
    b&=r_1\cdots r_n\\
  ...
  これを$ab=pk$に代入すると、
  \[
    q_1\cdots q_mr_1\cdots r_n=ps_1\cdots s_l
  \]
  となる。
  しかしUFDの定義から、$p$は$q_1,\dots q_m$、$r_1,\dots,r_n$のどれかと単元倍を除いて一致するはずである。
  もし$p$が$q_1,\dots,q_m$のどれかと単元倍を除いて一致すれば、すなわち$a\in(p)$となる。
  $p$が$r_1,\dots,r_n$のどれかと単元倍を除いて一致すれば、$b\in(p)$となる。
  ゆえに$(p)$は素イデアルであるから、$p$は素元である。
\end{proof}

UFDでの元の既約元への分解は「素元分解」と呼ぶことが多いようです。
また、邦書の多くではUFDを「0でも単元でもない元が素元に分解されるような整域」として定義されており、分解の一意性は定義に含まれていません。
このような立場をとる場合、素元分解の一意性は証明しなければならない定理になります。
