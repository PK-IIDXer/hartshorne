\chapter{可換環論 初級}\label{chapter-commutative_rings_first_course}

こちらは$\text{Euclid環}\subset\text{PID}$の紹介と証明から、Noether環に至るまでを中心に、その他もろもろ解説していきます。
可能な限り、それらが必要となった背景知識も盛り込んでいきます。

前提知識は第\ref{chapter-commutative_rings_introduction}章ぐらいを想定しています。
つまり、可換環の定義を知っていて、環準同型や準同型定理が分かっていると良いです。
Noether環については\mycite{Hot21}の第4章が参考になりますが、このあたりから\mycite{At-Mc-translate}に手を付け始めてもよいでしょう。

\section{Euclid環}

第\ref{chapter-commutative_rings_introduction}章の最後の例題で、\textbf{多項式の割り算}をしれっと行いました。
これは高校数学でもやったことあると思います。
つまり整数の時と同じように、多項式$f$を$g$で割るという操作
\[
...
\begin{definition}
  整域$A$が\textbf{Euclid環}(\textit{Euclidean domain}\footnote{
    直訳すると「Euclid整域」になりそうですが、最近の本はEuclid環と訳すのが主流のようで。
  })であるとは、写像
  \[
