...
\begin{definition}[Noether環]
  任意のイデアルが有限生成であるような環$A$を\textbf{Noether環}(\textit{Noetherian ring})と呼ぶ。
\end{definition}
...
整域、ユークリッド環、PID、Noether環と色々な環を紹介しましたが、まだ特筆すべき種類の環があります。
...
UFDを既約元分解の一意性で定義すると、既約元が素元になることを証明しなければならなくなります。
UFDを素元分解の可能性で定義すると、素元分解の一意性が従い、さらに従って既約元が素元であることが示さなければならないです。
いずれにせよ、どちらで定義しても同じです。
キバらず先に進みましょう。

\subsubsection{PIDはUFD}
今まで紹介してきた環の中で、Euclid環は
